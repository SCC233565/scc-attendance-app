...
function FinanceDashboard({ isOwner, onLock }) {
  ...
  const [txForm, setTxForm] = useState({ category_id: "", amount: "", description: "", transaction_date: new Date().toISOString().slice(0,10), payment_method: "Cash" });
  ...
  const load = async () => {
    setLoading(true);
    const [{ data: cats }, { data: txs }, { data: buds }, { data: audit }] = await Promise.all([
  ...
  const addTransaction = async () => {
    ...
    const { data: inserted } = await supabase.from("finance_transactions").insert([{
      ...
      description: txForm.description, transaction_date: txForm.transaction_date, payment_method: txForm.payment_method, created_by: ud?.user?.id
    }]).select().single();
    if (inserted) {
    ...
    setTxForm({ category_id: "", amount: "", description: "", transaction_date: new Date().toISOString().slice(0,10), payment_method: "Cash" });
  ...
  const openEditTransaction = (t) => setEditTx({
    ...
    transaction_date: t.transaction_date, payment_method: t.payment_method || "Cash", type: t.type
  });
                  ...
                  return (
                    <div key={c.id} className="flex items-center justify-between gap-3 px-4 py-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div onClick={() => saveBudget(c.id)} className="text-xs bg-[#4A0E52] text-white rounded-md px-2.5 py-1.5 cursor-pointer">Save</div>
                      </div>
                    </div>
                  );
                })}
          ...
          {tab === "reports" && (
            <div>
              <div className="flex justify-end gap-2 mb-4">
                <div onClick={exportPDF} className="text-xs border border-[#4A0E52] text-[#4A0E52] rounded-md px-3 py-2 cursor-pointer flex items-center gap-1.5"><FileText className="w-3.5 h-3.5" /> Export PDF</div>
                <div onClick={exportExcel} className="text-xs border border-[#4A0E52] text-[#4A0E52] rounded-md px-3 py-2 cursor-pointer flex items-center gap-1.5"><FileSpreadsheet className="w-3.5 h-3.5" /> Export Excel</div>
              </div>
