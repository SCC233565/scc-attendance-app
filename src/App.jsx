    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-40">
      <div className="bg-white rounded-xl p-6 w-full max-w-sm shadow-2xl">
        <div className="w-12 h-12 bg-[#FEE2E2] rounded-full flex items-center justify-center mx-auto mb-4">
          <Trash2 className="w-6 h-6 text-[#A6423A]" />
        </div>
        <h2 className="font-display text-lg text-center text-[#1F1F1F] mb-1">Delete {type}?</h2>
        <p className="text-sm text-center text-[#6B7280] mb-6">
          Are you sure you want to delete <span className="font-medium text-[#1F1F1F]">{name}</span>? You will have 5 seconds to undo.
        </p>
        <div className="flex gap-3">
          <div onClick={onCancel} className="flex-1 border border-[#E9E2CC] rounded-lg py-2.5 text-center text-sm cursor-pointer font-medium">
            Cancel
          </div>
          <div onClick={onConfirm} className="flex-1 bg-[#A6423A] text-white rounded-lg py-2.5 text-center text-sm cursor-pointer font-medium">
            Yes, Delete
          </div>
        </div>
      </div>
...
function UndoToast({ message, onUndo, onDismiss }) {
  ...
  return (
    <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-50 w-80 shadow-2xl rounded-xl overflow-hidden">
      <div className="bg-[#1F1F1F] text-white px-4 py-3 flex items-center gap-3">
        <span className="text-sm flex-1">{message}</span>
        <div onClick={onUndo} className="text-[#F3D98B] text-sm font-bold cursor-pointer whitespace-nowrap">UNDO</div>
        <div onClick={onDismiss} className="cursor-pointer text-[#9CA3AF]"><X className="w-4 h-4" /></div>
      </div>
      <div className="bg-[#374151] h-1">
        <div className="bg-[#F3D98B] h-1 transition-all duration-100" style={{width: progress + "%"}} />
      </div>
    </div>
  );
}
...
function MembersView({ members, refresh, isAdmin }) {
  ...
  const handleUndo = async () => {
    if (!undoData) return;
    const { id, created_at, created_by, ...data } = undoData;
  ...
  return (
    <div>
          ...
          <div key={m.id} className="flex items-center justify-between px-4 py-3">
            ...
            {isAdmin && (
              <div className="flex gap-2">
                ...
                <div onClick={() => requestDelete(m)} className="p-2 cursor-pointer text-[#A6423A]"><Trash2 className="w-4 h-4" /></div>
              </div>
            )}
          </div>
        ))}
      ...
      {undoData && (
        <UndoToast
          message={`"${undoData.full_name}" deleted.`}
          onUndo={handleUndo}
...
function StaffView() {
  ...
  const requestDeleteStaff = (person) => {
    setConfirmStaff(person);
  };

  const removeStaff = async () => {
    const person = confirmStaff;
            ...
            <div key={s.id} className="flex items-center justify-between px-4 py-3 flex-wrap gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                {s.id === currentUserId ? (
                  ...
                  <div onClick={() => requestDeleteStaff(s)} className="p-1 cursor-pointer text-[#A6423A]">
