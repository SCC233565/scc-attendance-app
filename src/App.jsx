...
/* ------------------------------ Reports View ------------------------------ */
function ReportsView({ members, selectedMemberId, onBackToAllReports }) {
  ...
  const memberRecords = useMemo(
    () => (selectedMemberId ? filteredRecords.filter((r) => r.member_id === selectedMemberId) : []),
    [filteredRecords, selectedMemberId]
  );
  ...
  const memberByService = useMemo(() => {
    ...
    memberRecords.forEach((r) => { map[r.service_type] = (map[r.service_type] || 0) + 1; });
    return Object.entries(map).map(([name, count]) => ({ name, count }));
  }, [memberRecords, selectedMemberId]);

  const memberRate = totalSessions ? Math.round((memberRecords.length / totalSessions) * 100) : 0;

  if (selectedMember) {
    return (
      <div>
        <div onClick={onBackToAllReports} className="flex items-center gap-2 text-sm text-[#4A0E52] cursor-pointer mb-4">
          ‹ Back to all reports
        </div>
        <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
          <h1 className="font-display text-2xl text-[#4A0E52]">{selectedMember.full_name}</h1>
          <DateRangePicker range={range} onChange={setRange} />
        </div>
        <p className="text-sm text-[#9CA3AF] mb-6">
          {selectedMember.membership_status} · {selectedMember.phone || "no phone"} · {selectedMember.email || "no email"}
        </p>

        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-lg border border-[#E9E2CC] p-4">
            <p className="text-xs text-[#9CA3AF]">Services Attended</p>
            <p className="text-2xl font-display text-[#4A0E52]">{memberRecords.length}</p>
          </div>
          <div className="bg-white rounded-lg border border-[#E9E2CC] p-4">
            <p className="text-xs text-[#9CA3AF]">Attendance Rate</p>
            <p className="text-2xl font-display text-[#4A0E52]">{memberRate}%</p>
          </div>
          <div className="bg-white rounded-lg border border-[#E9E2CC] p-4">
            <p className="text-xs text-[#9CA3AF]">Member Since</p>
            <p className="text-2xl font-display text-[#4A0E52]">{selectedMember.date_joined || "—"}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-[#E9E2CC] p-6">
          ...
          {memberRecords.length === 0 ? (
            <p className="text-sm text-[#9CA3AF]">No attendance recorded in this range.</p>
          ) : (
            <ul className="divide-y divide-[#F1ECDE]">
              {[...memberRecords].reverse().map((r) => (
...
export default function App() {
  ...
  return (
    <Shell view={view} setView={navigate} isAdmin={isAdmin} signOut={signOut}>
      ...
      {view === "members" && (
        <MembersView
          ...
          isAdmin={isAdmin}
          onSelectMember={(id) => { setSelectedMemberId(id); setView("reports"); }}
        />
      )}
      {view === "reports" && (
        <ReportsView
          ...
          onBackToAllReports={() => setSelectedMemberId(null)}
        />
      )}
