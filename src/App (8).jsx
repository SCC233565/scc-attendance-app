  ...
  const save = async () => {
    ...
    if (rows.length) {
      ...
      try {
        ...
        if (error) {
          // The server actually responded with a rejection — this is a real error,
          // not a connectivity problem, so show it instead of pretending it's "offline".
          setSaving(false);
          alert("Could not save attendance: " + error.message);
          return;
        }
      } catch (e) {
        // A thrown exception here means the request never reached the server at all —
        // genuine network failure, safe to queue for automatic retry.
        addToOfflineQueue(rows);
        setSaving(false); setSavedOffline(true);
        setTimeout(() => setSavedOffline(false), 4000);
        return;
...
export default function App() {
  ...
  const refreshMembers = useCallback(async () => {
    const [{ data: mems }, { data: mds }] = await Promise.all([
      supabase.from("members").select("*").eq("archived", false).order("full_name", { ascending: true }),
